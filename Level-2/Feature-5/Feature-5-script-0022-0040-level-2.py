...
class TC0050001(unittest.TestCase):
    ...
    def test_t_c0050001(self):
        ...
        # read files
        with open('Feature-5-data-0022-0040-level-2.csv', mode='r', encoding='utf-8-sig') as csvfile:
            ...
            for row in reader:
                ...
                try:
                    ...
                    driver.find_element(self.config['Subject']['type'], self.config['Subject']['value']).send_keys("Inquiry subject")
                    driver.find_element(self.config['Message']['type'], self.config['Message']['value']).send_keys("I have a question")
                    driver.find_element(self.config['Name']['type'], self.config['Name']['value']).send_keys("Nguyen Hai Dang")
                    driver.find_element(self.config['Email']['type'], self.config['Email']['value']).send_keys(row['email'])
